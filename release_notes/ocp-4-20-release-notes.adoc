...
* When using a Time Stamp Counter (TSC) configuration, the ts2phc metrics are incorrectly reported as unlocked instead of ptp4l. As a result, users may experience inaccurate Precision Time Protocol (PTP) clock state reporting. To work around this issue, remove the ts2phc and ptp4l metrics. (link:https://issues.redhat.com/browse/OCPBUGS-63158[OCPBUGS-63158])

* When updating the iDRAC firmware, an insufficient user delay setting in the BIOS can cause the update to fail. This results in an iDRAC reboot error and service interruption. To resolve this problem, disable SR-IOV and increase the AC Power Recovery User Delay. Reboot the system and attempt the update again. (link:https://issues.redhat.com/browse/OCPBUGS-60876[OCPBUGS-60876])
