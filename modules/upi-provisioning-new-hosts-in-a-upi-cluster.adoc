...
[NOTE]
====
When provisioning new bare-metal hosts to the cluster using the BMO, the `spec.externallyProvisioned` specification in the `BareMetalHost` custom resource is set to `false` by default.
====
...
<8> Optional: Replace `<root_device_hint>` with a device path if you specify a root device hint.
--
+
...
<5> Optional: Replace `<root_device_hint>` with a device path if you specify a root device hint. See "Root device hints" for additional details.
+
[IMPORTANT]
====
...
<num>:: Specifies the worker number.

. Inspect the bare-metal node:
+
[source,terminal]
----
